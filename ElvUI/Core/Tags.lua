...
local hex
-- COLORTEST = nil
ElvUF.Tags.Events["name:ilvl"] = 'UNIT_FACTION UNIT_TARGET UNIT_CONNECTION PLAYER_FLAGS_CHANGED UNIT_NAME_UPDATE'
ElvUF.Tags.Methods["name:ilvl"] = function(unit)
    local ilvl = ItemLevelMixIn:GetItemLevel(UnitGUID(unit))
    if not ilvl then return "-_-" end
    local color = GetItemLevelColor(ilvl)
    if not color then return tostring(math.floor(ilvl + 0.5)) end
    local hex = E:RGBToHex(color.r, color.g, color.b)
    return string.format("%s%d|r", hex, math.floor(ilvl + 0.5))
end
...
E.TagInfo = {
	...
	["name:ilvl"] = {category = "ilvl", description = "Displays ilvl"},
}
