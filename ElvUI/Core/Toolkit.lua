...
local function safe_unpack(tbl, n)
    if type(tbl) == "table" then return unpack(tbl) end
    if n == 4 then return 0, 0, 0, 1 end
    return 0, 0, 0
end
...
local function GetTemplate(template, isUnitFrameElement)
    ...
    if template == "ClassColor" then
        local color = (CUSTOM_CLASS_COLORS and CUSTOM_CLASS_COLORS[E.myclass]) or (RAID_CLASS_COLORS and RAID_CLASS_COLORS[E.myclass]) or {r=1,g=1,b=1}
        borderr, borderg, borderb = color.r, color.g, color.b
        backdropr, backdropg, backdropb = safe_unpack(media.backdropcolor, 3)

    elseif template == "Transparent" then
        local bc = (isUnitFrameElement and media.unitframeBorderColor) or media.bordercolor
        borderr, borderg, borderb = safe_unpack(bc, 3)
        backdropr, backdropg, backdropb, backdropa = safe_unpack(media.backdropfadecolor, 4)
...
local function SetOutside(obj, anchor, xOffset, yOffset, anchor2)
    ...
    local s = (type(E.Spacing) == "number") and E.Spacing or 0
    local x = (type(xOffset) == "number") and xOffset or s
    local y = (type(yOffset) == "number") and yOffset or s

    anchor  = anchor  or obj:GetParent() or obj
    anchor2 = anchor2 or anchor

    obj:ClearAllPoints()
    obj:SetPoint("TOPLEFT",     anchor,  "TOPLEFT",     -x,  y)
    obj:SetPoint("BOTTOMRIGHT", anchor2, "BOTTOMRIGHT",  x, -y)
end

local function SetInside(obj, anchor, xOffset, yOffset, anchor2)
    if not obj then return end

    local s = (type(E.Spacing) == "number") and E.Spacing or (E.Border or 0)
    local x = (type(xOffset) == "number") and xOffset or s
    local y = (type(yOffset) == "number") and yOffset or s

    anchor  = anchor  or (obj.GetParent and obj:GetParent()) or obj
    anchor2 = anchor2 or anchor

    if obj.GetPoint and obj:GetPoint() then
        obj:ClearAllPoints()
    end
...
local function SetTemplate(frame, template, glossTex, ignoreUpdates, forcePixelMode, isUnitFrameElement)
	...
	local bgFile = glossTex and (E.media and E.media.glossTex) or (E.media and E.media.blankTex) or "Interface\\Buttons\\WHITE8x8"
	local edge   = (E.media and E.media.blankTex) or "Interface\\Buttons\\WHITE8x8"
	...
	if template ~= "NoBackdrop" then
		...
		if not E.PixelMode and not frame.forcePixelMode then
			if not frame.iborder then
				...
				border:SetBackdrop({ edgeFile = edge, edgeSize = mult, insets = {left = -mult, right = -mult, top = -mult, bottom = -mult} })
